...
[hub]: https://hub.docker.com/r/parana/jessie-ldap/

Latest release: 1.1.2 - OpenLDAP 2.4.40 -  [Changelog](CHANGELOG.md) | [Docker Hub](https://hub.docker.com/r/parana/jessie-ldap/) 
...
- [Advanced User Guide](#advanced-user-guide)
	- [Extend parana/jessie-ldap:1.1.2 image](#extend-osixiaopenldap112-image)
...
Run OpenLDAP docker image:
	...
	docker run --name my-openldap-container --detach parana/jessie-ldap:1.1.2
...
By default the admin has the password **admin**. All those default settings can be changed at the docker command line, for example:
	...
	--env LDAP_ADMIN_PASSWORD="JonSn0w" --detach parana/jessie-ldap:1.1.2
...
simply mount this directories as a volume to `/var/lib/ldap` and `/etc/ldap/slapd.d`:
	...
	--detach parana/jessie-ldap:1.1.2
...
> [parana/jessie-ldap-backup](https://github.com/osixia/docker-openldap-backup)
...
By default TLS is enable, a certificate is created with the container hostname (it can be set by docker run --hostname option eg: ldap.example.org).
	...
	docker run --hostname ldap.my-company.com --detach parana/jessie-ldap:1.1.2
...
Add --env LDAP_TLS=false to the run command:
	...
	docker run --env LDAP_TLS=false --detach parana/jessie-ldap:1.1.2

### Multi master replication
Quick example, with the default config.

	#Create the first ldap server, save the container id in LDAP_CID and get its IP:
	LDAP_CID=$(docker run --hostname ldap.example.org --env LDAP_REPLICATION=true --detach parana/jessie-ldap:1.1.2)
	LDAP_IP=$(docker inspect -f "{{ .NetworkSettings.IPAddress }}" $LDAP_CID)

	#Create the second ldap server, save the container id in LDAP2_CID and get its IP:
	LDAP2_CID=$(docker run --hostname ldap2.example.org --env LDAP_REPLICATION=true --detach parana/jessie-ldap:1.1.2)
...
To fix that run the container with `--copy-service` argument :
		...
		docker run [your options] parana/jessie-ldap:1.1.2 --copy-service
...
Example command to run the container in `debug` mode:
	...
	docker run --detach parana/jessie-ldap:1.1.2 --loglevel debug

See all command line options:

	docker run parana/jessie-ldap:1.1.2 --help
...
- **LDAP_REPLICATION_HOSTS**: list of replication hosts, must contain the current container hostname set by --hostname on docker run command. Defaults to :
  ...
  - ldap://ldap2.example.org
	...
	If you want to set this variable at docker run command add the tag `#PYTHON2BASH:` and convert the yaml in python:
		...
		docker run --env LDAP_REPLICATION_HOSTS="#PYTHON2BASH:['ldap://ldap.example.org','ldap://ldap2.example.org']" --detach parana/jessie-ldap:1.1.2
...
### Extend parana/jessie-ldap:1.1.2 image

If you need to add your custom TLS certificate, bootstrap config or environment files the easiest way is to extends this image.

Dockerfile example:

	FROM parana/jessie-ldap:1.1.2
